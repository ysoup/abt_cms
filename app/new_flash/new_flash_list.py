...
from app.models import NewFlashInformation, NewFlashCategory, NewFlashRule, AccountManage
...
# # 快讯列表页
# @new_flash.route('/new_flash_list', methods=['GET'])
# @login_required
# def new_flash_list():
#     try:
#         page = request.args.get('page', 1, type=int)
...
#             if select_id == 2:
#                 pagination = NewFlashInformation.query.filter(NewFlashInformation.is_delete==0,
#                                                               NewFlashInformation.source_name.notin_([ORIGINAL, BGMARKET]))\
#                     .order_by(NewFlashInformation.create_time.desc()).paginate(page, per_page=10, error_out=False)
#             elif select_id == 3:
#                 pagination = NewFlashInformation.query.filter_by(source_name=ORIGINAL).\
#                     order_by(NewFlashInformation.create_time.desc()).paginate(page, per_page=10, error_out=False)
#             elif select_id == 4:
#                 # pagination = NewFlashInformation.query.filter_by(is_delete=0, source_name=BGMARKET).order_by(NewFlashInformation.create_time.desc()).paginate(page, per_page=10, error_out=False)
#                 pagination = NewFlashInformation.query.filter_by(is_delete=0, is_show=0).\
#                     order_by(NewFlashInformation.create_time.desc()).paginate(page, per_page=10, error_out=False)
#             elif select_id == 1:
#                 pagination = NewFlashInformation.query.filter_by(is_delete=0, is_show=1).\
#                     order_by(NewFlashInformation.create_time.desc()).paginate(page, per_page=10, error_out=False)
#             elif select_id == 0:
#                 pagination = NewFlashInformation.query.filter_by(is_delete=0).\
#                     order_by(NewFlashInformation.create_time.desc()).paginate(page, per_page=10, error_out=False)
#             else:
#                 return render_template('404.html')
#         else:
...
#                 order_by(NewFlashInformation.create_time.desc()).paginate(page, per_page=10, error_out=False)
#         data = pagination.items
#         since_obj = NewFlashInformation.query.filter_by(is_delete=0).\
#             order_by(NewFlashInformation.create_time.desc()).first()
#         new_id = since_obj.id if since_obj.id else 0
...
#         return render_template('new_flash/new_flash_list.html', data=data, pagination=pagination,
#                                modal_data=modal_data, news_since_id = new_id)
#     except Exception as e :
#         current_app.logger.error(e)
#         return render_template("404.html")
#
